...
public class AppIndependentmethods extends DriverScripts{
	 ...
	 * 
	 ***************************************/
	public boolean setObject(WebDriver oDriver, String strObjectName, Keys strData)
	{
		WebElement oEle = null;
		try {
			oEle = appInd.createAndGetObject(oDriver, strObjectName);
	...
	public boolean selectDropDownValue(WebDriver oDriver, By locator, int option )
	{
		Select dropdown = null;
		try
		{
			dropdown = new Select(oDriver.findElement(locator));
            dropdown.selectByIndex(option);
            appInd.writeReport("pass", "dropdown selection passed");
            return true;
		}catch(Exception e)
		{
			appInd.writeReport("exception", "exception while executing 'selectDropDownValue' method "+e.getMessage());
			return false;
		}
		finally
		{
